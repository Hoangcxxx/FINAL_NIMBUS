...
public class HoaDonService {
    ...
    public List<HoaDonDTO> hienthi(String maHoaDon) {
        ...
        if (maHoaDon != null && !maHoaDon.isEmpty()) {
            // Tìm hóa đơn theo mã
            hoaDonList = hoaDonRepository.findByMaHoaDon(maHoaDon).map(Collections::singletonList).orElse(Collections.emptyList());
        } else {
            // Lấy toàn bộ hóa đơn
    ...
    private HoaDonDTO convertToDTO(HoaDon hoaDon) {
        ...
        if (hoaDon.getVoucher() != null) {
            ...
            // Kiểm tra kiểu giảm giá (0: %, 1: giá tiền)
            dto.setKieugiamgia(hoaDon.getVoucher().getKieuGiamGia());
        } else {
            dto.setGiaTriMavoucher(BigDecimal.ZERO); // Giá trị mặc định khi không có voucher
            dto.setMavoucher(null);
            dto.setKieugiamgia(null); // Không có kiểu giảm giá
        }
        ...
        if (hoaDon.getPhuongThucThanhToanHoaDon() != null) {
            ...
            dto.setTenPhuongThucThanhToan(hoaDon.getPhuongThucThanhToanHoaDon().getPhuongThucThanhToan().getTenPhuongThuc());
        }

        // Lấy danh sách sản phẩm chi tiết
        List<HoaDonChiTiet> chiTietList = hoaDonChiTietRepository.findByHoaDon_IdHoaDon(hoaDon.getIdHoaDon());
        dto.setListSanPhamChiTiet(chiTietList.stream().map(chiTiet -> {
        ...
        List<LichSuThanhToan> lichSuThanhToanList = lichSuThanhToanRepository.findByHoaDon_IdHoaDon(hoaDon.getIdHoaDon());

        List<LichSuThanhToanDTO> lichSuThanhToanDTOList = lichSuThanhToanList.stream().map(lichSu -> {
            LichSuThanhToanDTO lichSuDTO = new LichSuThanhToanDTO();
            lichSuDTO.setId(lichSu.getIdLichSuHoaDon()); // Đảm bảo getIdLichSuHoaDon trả về giá trị
            lichSuDTO.setTrangThaiThanhToan(lichSu.getTrangThaiThanhToan());
            return lichSuDTO;
        }).collect(Collectors.toList());

// Set vào DTO hóa đơn
        dto.setIdlichsuhoadon(lichSuThanhToanDTOList);
...
// Trạng thái "Chờ xử lý" sau khi thanh toán được chọn
    ...
    private void saveVNPayPayment(HoaDon hoaDon, String generatedMaHoaDon, HoaDonDTO hoaDonDTO, HttpServletRequest req, HttpServletResponse resp) {
        try {
...
//            resp.sendRedirect(paymentUrl);
            ...
            // Không lưu hóa đơn ngay tại đây, sẽ lưu sau khi nhận thông báo thanh toán từ VNPay
        } catch (Exception e) {
    ...
    public String createOrder(HoaDonDTO hoaDonDTO, HttpServletRequest res, HttpServletResponse res1) {
        ...
        phiVanChuyen.setTrangThai(true); // Phí vận chuyển còn hiệu lực
        phiVanChuyen.setSoTienVanChuyen(phiVanChuyen.getSoTienVanChuyen()); // Phí vận chuyển là 25.000 VNĐ
        phiVanChuyen.setNgayTao(new Date());
        phiVanChuyen.setDiaChiVanChuyen(diaChiVanChuyen);
        phiVanChuyen.setNgayCapNhat(new Date());
        phiVanChuyen.setMoTa("Phí vận chuyển cho đơn hàng");



// Gán phí vận chuyển vào hóa đơn
        hoaDon.setPhiShip(phiVanChuyen.getSoTienVanChuyen());  // Gán đối tượng PhiVanChuyen vào hóa đơn
...
// Kiểm tra nếu idvoucher không phải null trước khi thực hiện các thao tác
        ...
        switch (hoaDonDTO.getTenPhuongThucThanhToan()) {
            case "cod":
                ...
                break;
            case "vnpay":
                saveVNPayPayment(hoaDon, generatedMaHoaDon, hoaDonDTO, res, res1);
//                loaiTrangThai = loaiTrangThaiRepository.findById(2).orElseThrow(() -> new IllegalArgumentException("Loại trạng thái không tồn tại!"));
                break;
            case "zalopay":
    ...
    public HoaDon createHoaDon(HoaDon hoaDon) {
        ...
        LoaiTrangThai loaiTrangThai1 = loaiTrangThaiRepository.findById(1).orElseThrow(() -> new IllegalArgumentException("Loại trạng thái không tồn tại!"));
        ...
        LoaiTrangThai loaiTrangThai2 = loaiTrangThaiRepository.findById(4).orElseThrow(() -> new IllegalArgumentException("Loại trạng thái không tồn tại!"));
    ...
    public HoaDon getInvoiceById(Integer id) {
        ...
        return hoaDonRepository.findById(id).orElseThrow(() -> new NoSuchElementException("Không tìm thấy hóa đơn với ID: " + id));
    }
    ...
    public HoaDonResponseDTO updateHoaDon(int idHoaDon, HoaDonUpdateDTO updateHoaDonDTO) {
        ...
        PhuongThucThanhToanHoaDon existingPtThanhToan = ptThanhToanHoaDonRepository.findById(ptThanhToan.getIdThanhToanHoaDon()).orElseThrow(() -> new RuntimeException("Phương thức thanh toán không tồn tại."));
        hoaDon.setPhuongThucThanhToanHoaDon(existingPtThanhToan);

        if (updateHoaDonDTO.getIdVoucher() > 0) {
            Voucher voucher = voucherRepository.findById(updateHoaDonDTO.getIdVoucher()).orElseThrow(() -> new RuntimeException("Voucher không tồn tại"));
            hoaDon.setVoucher(voucher);
        }

        if (!existingPtThanhToan.getPhuongThucThanhToan().getIdPTThanhToan().equals(2) && !existingPtThanhToan.getPhuongThucThanhToan().getIdPTThanhToan().equals(3)) {
            ...
            LoaiTrangThai loaiTrangThai1 = loaiTrangThaiRepository.findById(5).orElseThrow(() -> new RuntimeException("Loại trạng thái không tồn tại"));
            trangThaiHoaDon1.setLoaiTrangThai(loaiTrangThai1);
            ...
            LoaiTrangThai loaiTrangThai2 = loaiTrangThaiRepository.findById(8).orElseThrow(() -> new RuntimeException("Loại trạng thái không tồn tại"));
            trangThaiHoaDon2.setLoaiTrangThai(loaiTrangThai2);
        ...
        return new HoaDonResponseDTO(hoaDon.getPhiShip(), hoaDon.getNgayThanhToan(), hoaDon.getThanhTien(), hoaDon.getPhuongThucThanhToanHoaDon().getIdThanhToanHoaDon());
    }
    ...
    public List<HoaDonChiTietDTO> createMultipleHoaDonChiTiet(List<HoaDonChiTietDTO> dtoList, Integer userId) {
        ...
        for (HoaDonChiTietDTO dto : dtoList) {
            HoaDonChiTiet hoaDonChiTiet = new HoaDonChiTiet();
            // Tìm kiếm người dùng
            NguoiDung nguoiDung = nguoiDungRepository.findById(userId).orElseThrow(() -> new ExpressionException("Người dùng không tìm thấy"));
            SanPhamChiTiet sanPhamChiTiet = sanPhamChiTietRepository.findById(dto.getIdSanPhamChiTiet()).orElseThrow(() -> new ExpressionException("Sản phẩm chi tiết không tìm thấy"));
            hoaDonChiTiet.setSanPhamChiTiet(sanPhamChiTiet);
            HoaDon hoaDon = hoaDonRepository.findById(dto.getIdHoaDon()).orElseThrow(() -> new ExpressionException("Hóa đơn không tìm thấy"));
            GiamGiaSanPham giamGiaSanPham = giamGiaSanPhamRepository.findBySanPham(sanPhamChiTiet.getSanPham()).orElse(null);
            ...
            // Tính giá thực tế
            BigDecimal giaTinh = (giaKhuyenMai != null && giaKhuyenMai.compareTo(BigDecimal.ZERO) > 0) ? giaKhuyenMai : giaBan;
            ...
            hoaDonChiTietRepository.save(hoaDonChiTiet);
            HoaDonChiTietDTO createdDto = new HoaDonChiTietDTO(sanPhamChiTiet.getIdSanPhamChiTiet(), hoaDon.getIdHoaDon(), hoaDon.getMaHoaDon(), hoaDonChiTiet.getSoLuong(), hoaDonChiTiet.getTongTien(), hoaDonChiTiet.getSoTienThanhToan(), hoaDonChiTiet.getTienTraLai(), hoaDonChiTiet.getMoTa());
